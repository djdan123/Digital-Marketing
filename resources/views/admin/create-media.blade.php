...
@section('page_title', 'Ajouter un média')
...
<div class="bg-white border border-gray-200 rounded-lg shadow-sm dark:border-gray-700 dark:bg-gray-800 p-6">
    <form action="{{ route('admin.media.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <select name="company_id" class="w-full px-4 py-2 border rounded-lg bg-gray-50 border-gray-300 text-gray-900 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
                    ...
                    @foreach($companies as $company)
                        <option value="{{ $company->id }}"{{ old('company_id') == $company->id ? ' selected' : '' }}>{{ $company->display_name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
            ...
            <div>
                ...
                <select name="pricing_type" class="w-full px-4 py-2 border rounded-lg bg-gray-50 border-gray-300 text-gray-900 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
                    ...
                    @foreach($pricingTypes as $pricingType)
                        <option value="{{ $pricingType->value }}"{{ old('pricing_type') === $pricingType->value ? ' selected' : '' }}>{{ $pricingType->label() }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div>
            ...
            <textarea name="description" rows="4" class="w-full px-4 py-2 border rounded-lg bg-gray-50 border-gray-300 text-gray-900 dark:bg-gray-700 dark:border-gray-600 dark:text-white">{{ old('description') }}</textarea>
        </div>
        <div class="grid gap-4 md:grid-cols-2">
            <div>
                <label class="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Prix</label>
                <input type="number" step="0.01" name="base_price" value="{{ old('base_price') }}" class="w-full px-4 py-2 border rounded-lg bg-gray-50 border-gray-300 text-gray-900 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
            </div>
            <div>
        ...
        <div class="flex justify-end gap-2">
            ...
            <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-primary-700 rounded-lg hover:bg-primary-800">Enregistrer</button>
        </div>
    </form>
</div>
@endsection
